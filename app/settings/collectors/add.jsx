...
function AddCollectorScreen() {
  const { theme } = useTheme();
  const { colors, spacing, radius, shadows } = theme;
  const styles = useMemo(() => getStyles(theme), [theme]);
  const router = useRouter();
  const { addCollector } = useContext(CollectorContext);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const collectorId = await addCollector({
        name: name.trim(),
        mobile: mobile.trim(),
        area: area.trim(),
        status,
      ...
      if (collectorId) {
        ...
        setError("Failed to register collector in Firestore database.");
      }
    } catch (e) {
    ...
    <>
      ...
      <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.header}>
          <BackButton label="Cancel" onPress={() => router.push("/settings/collectors")} />
          ...
          >
            {saving ? (
              ...
              <>
                ...
                <Text style={styles.saveButtonText}>Register Collector</Text>
              </>
            )}
          </Pressable>
        </View>
...
export default function AddCollectorRoute() {
  return (
    <ProtectedRoute>
      <AddCollectorScreen />
    </ProtectedRoute>
