...
function AddItemScreen() {
  ...
  const { addItem, items, updateItem } = useContext(ItemContext);
    ...
    try {
      if (itemType === "product" && status === "Active") {
        const activeProducts = items.filter(
          (i) => i.itemType !== "raw_material" && (i.status || "Active") === "Active"
        );
      ...
      const itemId = await addItem({
        ...
        openingStockSlots: formattedSlots,
      ...
      if (itemId) {
        ...
        setError("Failed to save product in Firestore database.");
      }
    } catch (e) {
        ...
        >
          {saving ? (
            ...
            <>
              ...
              <Text style={styles.submitButtonText}>Create Catalog Item</Text>
            </>
          )}
        </Pressable>
      </View>
