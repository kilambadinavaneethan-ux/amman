  ...
  const handleSave = async () => {
    ...
    if (billingSystem === "Time-Based" && overtimeRate.trim()) {
      ...
      if (isNaN(overtimeNum) || overtimeNum < 0) {
        setError("Overtime Rate must be a valid positive number.");
        return;
      }
    }
    ...
    if (hasShifting) {
      if (loadingCost.trim()) {
        ...
        if (isNaN(loadingCostNum) || loadingCostNum < 0) {
          setError("Loading Cost must be a valid positive number.");
          return;
        }
      }
      if (unloadingCost.trim()) {
        unloadingCostNum = parseFloat(unloadingCost);
        if (isNaN(unloadingCostNum) || unloadingCostNum < 0) {
          setError("Unloading Cost must be a valid positive number.");
          return;
        }
      }
    }
    ...
    try {
      ...
      if (workerId) {
        ...
        setError("Failed to register worker in Firestore database.");
      }
    } catch (e) {
    ...
    <>
      ...
      <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.formCard}>
          ...
          <Pressable
            style={({ pressed }) => [styles.importContactsBtn, pressed && { opacity: 0.8 }]}
            onPress={() => setContactsModalVisible(true)}
          >
            <MaterialIcons name="contacts" size={18} color={colors.bg.card} />
          ...
          >
            ...
            <MaterialIcons
              name={showRoleDropdown ? "arrow-drop-up" : "arrow-drop-down"}
              size={24}
          ...
          {billingSystem === "Time-Based" ? (
            ...
            <>
              ...
              <TextInput
                value={perDayBagsCount}
                onChangeText={setPerDayBagsCount}
                placeholder="e.g. 50"
                placeholderTextColor={colors.text.muted}
                keyboardType="numeric"
                style={styles.input}
          ...
          <TextInput
            ...
            multiline
            numberOfLines={2}
            style={[styles.input, { height: 60, paddingVertical: 10 }]}
          />
          ...
          <View style={styles.segmentRow}>
            {["Active", "Inactive"].map((s) => {
              ...
              return (
                <Pressable
                  key={s}
                  style={[styles.segmentBtn, active && (s === "Active" ? styles.segmentBtnActiveGreen : styles.segmentBtnActiveRed)]}
                  onPress={() => setStatus(s)}
                >
          ...
          >
            {saving ? (
              ...
              <>
                ...
                <Text style={styles.saveBtnText}>Register Worker</Text>
              </>
            )}
          </Pressable>
        </View>
...
const getStyles = (theme) => {
  ...
  importContactsBtn: {
    ...
    backgroundColor: colors.accent.success, height: 42, borderRadius: 10, marginBottom: 4,
  },
  importContactsBtnText: { color: colors.bg.card, fontSize: 14, fontWeight: "700" },
